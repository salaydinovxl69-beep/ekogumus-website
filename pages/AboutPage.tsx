...
export function AboutPage() {
  ...
  // --- ЛОГИКА АВТОПРОЛИСТЫВАНИЯ ---
  useEffect(() => {
    if (!api || !isAutoPlaying) return;

    // Используем setInterval для автопролистывания
    const interval = setInterval(() => {
      api.scrollNext();
    }, 2000); 

    return () => clearInterval(interval);
  }, [api, isAutoPlaying]); // Зависимости: api и флаг isAutoPlaying

  // Отслеживание текущего слайда для индикаторов
  useEffect(() => {
    if (!api) return;

    setCurrent(api.selectedScrollSnap());

    // Обновление при пролистывании (пользователем или автоматически)
    api.on('select', () => {
      setCurrent(api.selectedScrollSnap());
    });
  ...
  // Возобновление автопролистывания, когда курсор убран
  ...
  const handleImageClick = (url: string) => {
    // window.open(url, '_blank') открывает указанный URL в новой вкладке/окне
      ...
      <section className="py-16 bg-transparent-content">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            ...
            <div
              className="relative"
              // Обработчики наведения на контейнер карусели
            ...
            >
              <Carousel
                ...
                setApi={setApi}
                opts={{
                  align: "start", // Изменил на "start" для лучшего отображения части следующего слайда
                  loop: true, // Обеспечивает плавное и непрерывное пролистывание
                  slidesToScroll: 1,
                }}
              >
                      ...
                      <div className="group p-1">
                        ...
                        <div className="relative">
                          <div
                            ...
                            onClick={() => handleImageClick(certificate.url)} // Добавляем обработчик клика
                          >
                            <ImageWithFallback
                              src={certificate.url}
                              alt={certificate.alt}
                ...
                {certificates.map((_, index) => (
                  <div
                    ...
                    className={`w-2 h-2 rounded-full transition-all duration-300 ${
                      // При loop: true current будет представлять индекс внутри эмуляции,
                      // что вполне подходит для индикаторов.
                      index === current % certificates.length
